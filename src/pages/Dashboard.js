...
// Helper function to build an absolute URL for the profile picture using BASE_URL
const getProfilePicUrl = (path) => {
  if (!path) {
    const defaultUrl = 'https://via.placeholder.com/200';
    console.log('Profile image URL (default):', defaultUrl);
    return defaultUrl;
  }
  const cleanedPath = path.replace(/\\/g, '/');
  const fullUrl = `${API_CONFIG.BASE_URL}/${cleanedPath}`;
...
const Dashboard = () => {
  ...
  // Show modal if profile completion is less than 100%
  useEffect(() => {
    if (user.profileCompletion < 100) {
      setShowModal(true);
    } else {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          console.log(data);
          console.log('profilePicture in token:', user.profilePic);
          setUser((prevState) => ({
            ...prevState,
            profilePic: data.profilePicture,
            profileCompletion: data.profileCompletion,
  ...
  // Compute the full URL for the profile picture once
  const fullUrl = getProfilePicUrl(user.profilePic);

  return (
    <DashboardContainer>
      <UserInfoWidget>
        <UserImage src={fullUrl} alt="Profile" />
        <UserName>
          {user.firstName} {user.lastName}
        </UserName>
        <UserRole>{user.role}</UserRole>
