...
/**
 * Aggregate profit data by property sub-type for a given inquiry type.
 */
const aggregateDataBySubType = (data, targetInquiryType) => {
  // Define property sub-types for each property category
  const structure = {
    residential: ["home", "apartment", "villas", "farmhouse", "others"],
    commercial: ["office", "shop", "warehouse", "factory", "others"],
    land: ["agricultural", "plot", "industrial", "others"]
  ...
  Object.keys(structure).forEach((propType) => {
    results[propType] = {};
    structure[propType].forEach((subType) => {
      results[propType][subType] = { allTime: 0, thisYear: 0, thisMonth: 0 };
    });
  });
  ...
  data.forEach((item) => {
    ...
    if (results[propertyKey]) {
      // Use sub-type if exists; otherwise, default to "others"
      const subTypeArray = structure[propertyKey];
      const pSubType = normalize(item.propertySubType);
      const chosenSubType = subTypeArray.includes(pSubType) ? pSubType : "others";
      results[propertyKey][chosenSubType].allTime += profit;
      if (year === currentYear) {
        results[propertyKey][chosenSubType].thisYear += profit;
        if (month === currentMonth) {
          results[propertyKey][chosenSubType].thisMonth += profit;
        }
      }
    }
  });
...
function BusinessVolumeDetail() {
  ...
  // Retrieve inquiryType from location state (e.g., "For Sale")
  const inquiryType = location.state?.inquiryType || '';
