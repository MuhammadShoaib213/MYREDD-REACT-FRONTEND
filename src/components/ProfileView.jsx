...
//       try {
//         const response = await axios.get(`${API_CONFIG.API_URL}/auth/profile/${userId}`);
...
//     try {
//       await axios.patch(`${API_CONFIG.API_URL}/auth/profile/${userId}`, formData, config);
...
const ProfileView = () => {
  const [activeTab, setActiveTab] = useState('about');
  const [profile, setProfile] = useState({
    // Add your initial profile fields as needed
  });
  const [profileImage, setProfileImage] = useState(null);
  const [businessLogoImage, setBusinessLogoImage] = useState(null);
  const [error, setError] = useState(null);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const userId = decoded.userId;
      try {
        // API requests use the API URL from API_CONFIG
        const response = await axios.get(`${API_CONFIG.API_URL}/auth/profile/${userId}`);
        setProfile({ ...profile, ...response.data });
        setError(null);
      } catch (error) {
  ...
  return (
    <ProfileContainer>
      ...
      <LeftPanel>
        ...
        <ProfilePic
          // For images, use the BASE_URL from API_CONFIG
          src={profile.profilePicture 
            ? `${API_CONFIG.BASE_URL}/${profile.profilePicture}` 
            : 'https://via.placeholder.com/200'} 
          alt={profile.profilePicture} 
        />
        <FileInput type="file" onChange={(e) => handleImageChange(e, setProfileImage)} />
