...
const RightSidebar = () => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await axios.get(` http://localhost:6003/api/auth/search`, {
  ...
  const handleFriendRequest = async (userId) => {
    try {
      await axios.post(
        ' http://localhost:6003/api/friend/request',
        { recipientId: userId },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleInvite = async (event) => {
    ...
    try {
      if (inviteMethod === 'email') {
        ...
        await axios.post(
          ' http://localhost:6003/api/auth/invite',
          { email: inviteEmail },
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        await axios.post(
          ' http://localhost:6003/api/auth/invite-sms',
        ...
        searchResults.map((user) => (
          <UserCard key={user._id}>
            <ProfilePic
              src={
                user.profilePicture
                  ? ` http://localhost:6003/${user.profilePicture}`
