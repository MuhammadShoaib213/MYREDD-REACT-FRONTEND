...
// Build a full URL for images using the base URL
...
const ContentContainer = styled.div`
  ...
  @media (max-width: 768px) {
    flex-direction: column; // Change to column on smaller screens
    align-items: center; // Center align items for single column layout
  }
`;
...
const FriendDetail = () => {
  ...
  useEffect(() => {
    const fetchFriendDetail = async () => {
      try {
        // Use API_CONFIG.API_URL for API requests
        const response = await fetch(`${API_CONFIG.API_URL}/friend/detail/${id}`, {
          headers: {
  ...
  return (
    <PageContainer>
      ...
      <ContentContainer>
        <ProfileCard>
          <ProfileImage 
            src={friend.profilePicture ? getFullUrl(friend.profilePicture) : 'https://via.placeholder.com/150'} 
            alt={friend.fullName} 
          />
          <ProfileDetails>
            <DetailText>
            ...
            <Divider />
            <DetailText>
              <Label>Mobile:</Label> {friend.phoneNumber}
            </DetailText>
            <Divider />
            <DetailText>
              <Label>Age:</Label> {friend.age} Years
            </DetailText>
            <Divider />
            <DetailText>
              <Label>Member Since:</Label> {new Date(friend.createdAt).toLocaleDateString()}
            </DetailText>
            <Divider />
          </ProfileDetails>
        </ProfileCard>
        <TabsCard>
          <TabsHeader>About / Business Info</TabsHeader>
          <Tabs>
            <TabButton className={activeTab === 'about' ? 'active' : ''} onClick={() => setActiveTab('about')}>
              About
            </TabButton>
            <TabButton className={activeTab === 'business' ? 'active' : ''} onClick={() => setActiveTab('business')}>
              Business Info
            </TabButton>
          </Tabs>
          <TabContent>
            {activeTab === 'about' && (
              <InfoSection>
                <InfoItem>
                  <Label>City:</Label> {friend.city}
                </InfoItem>
                <InfoItem>
                  <Label>Mobile Primary:</Label> {friend.phoneNumber}
                </InfoItem>
                <InfoItem>
                  <Label>Mobile Secondary:</Label> {friend.whatsappNumber}
                </InfoItem>
                <InfoItem>
                  <Label>Email:</Label> {friend.email}
                </InfoItem>
                <InfoItem>
                  <Label>Location:</Label> {friend.location}
                </InfoItem>
              </InfoSection>
            )}
            {activeTab === 'business' && (
              <InfoSection>
                <InfoItem>
                  <Label>Business Name:</Label> {friend.businessName}
                </InfoItem>
                <InfoItem>
                  <Label>Business Owner Name:</Label> {friend.businessOwnerName}
                </InfoItem>
                <InfoItem>
                  <Label>Business Working Area:</Label> {friend.businessWorkingArea}
                </InfoItem>
                <InfoItem>
                  <Label>Residential:</Label> {friend.residential}
                </InfoItem>
                <InfoItem>
                  <Label>Commercial:</Label> {friend.commercial}
                </InfoItem>
                <InfoItem>
                  <Label>Land:</Label> {friend.land}
                </InfoItem>
                <InfoItem>
                  <Label>Experience:</Label> {friend.experience} Years
                </InfoItem>
                <InfoItem>
                  <Label>Skills:</Label> {friend.skills.join(', ')}
                </InfoItem>
              </InfoSection>
            )}
          </TabContent>
