...
// Aggregation function for profit calculations.
const aggregateData = (data) => {
  const volumes = {
    forsale: { 
      residential: { allTime: 0, thisYear: 0, thisMonth: 0 },
          ...
          <CategoryBlock key={index}>
            ...
            <Table>
              ...
              {Object.entries(details).map(([category, stats]) => (
                <TableRow key={category}>
                  <TableCell>{category}</TableCell>
                  <TableCell>{Number(stats?.allTime ?? 0).toLocaleString()}</TableCell>
                  <TableCell>{Number(stats?.thisYear ?? 0).toLocaleString()}</TableCell>
                  <TableCell>{Number(stats?.thisMonth ?? 0).toLocaleString()}</TableCell>
                </TableRow>
              ))}
            </Table>
            <LearnMoreButton onClick={() => handleNavigate(type)}>
