...
const AgentList = () => {
  ...
  const fetchAgents = async () => {
    ...
    try {
      const response = await axios.get(` http://localhost:6003/api/auth/agents/${agencyId}`, {
  ...
  const deleteAgent = async (agentId) => {
    ...
    try {
      await axios.delete(` http://localhost:6003/api/auth/agents/${agencyId}/${agentId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const updateAgent = async (agentId) => {
    ...
    try {
      await axios.put(` http://localhost:6003/api/auth/agents/${agencyId}/${agentId}`, { firstName: updatedName }, {
        headers: { Authorization: `Bearer ${token}` }
      });
