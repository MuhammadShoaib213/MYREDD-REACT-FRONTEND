...
//     try {
//       const response = await fetch('http://195.179.231.102:6003/api/auth/forgot-password', {
//         method: 'POST',
//         headers: {
...
function ForgotPasswordForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Password reset email sent!');
        navigate('/OTPVerificationForm', { state: { email } }); // Pass email in state to the next component
      } else {
        toast.error(data.message);
      }
    } catch (error) {
