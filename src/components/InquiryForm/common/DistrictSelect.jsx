...
//           `http://195.179.231.102:6003/api/places/cities/${selectedCityName}/areas`
//         );
...
function DistrictSelect({ selectedCityName, area, onAreaChange, isRequired = false }) {
  ...
  useEffect(() => {
    ...
    const fetchAreas = async () => {
      try {
        const response = await axios.get(
          `${API_CONFIG.API_URL}/places/cities/${selectedCityName}/areas`
        );
  ...
  return (
    <>
      <Label htmlFor="area-select">
        District / Area {isRequired && <RequiredAsterisk>*</RequiredAsterisk>}
      </Label>
      <StyledSelectWrapper>
        <CreatableSelect
          id="area-select"
