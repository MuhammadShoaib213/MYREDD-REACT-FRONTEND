...
const LeadDetailPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        propertyResponse = await axios.get(` http://localhost:6003/api/properties/propertybyid/${id}`, { headers });
      ...
      if (propertyResponse && propertyResponse.data.customerId && token) {
        try {
          ...
          const notesResponse = await axios.get(` http://localhost:6003/api/notes/${userId}/${id}/${propertyResponse.data.customerId}`, { headers });
  ...
  const fetchSchedules = async () => {
    ...
    try {
      const decodedToken = jwtDecode(token);
      const userId = decodedToken.userId;
      const response = await axios.get(` http://localhost:6003/api/schedules/user/${userId}`, { headers });
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const response = await axios.patch(` http://localhost:6003/api/properties/updateStatus/${id}`, { status: newStatus }, { headers });
  ...
  const handleSaveNote = async () => {
    ...
    try {
      const response = await axios.post(' http://localhost:6003/api/notes', formData, {
  ...
  const handleDeleteNote = async (noteId, index) => {
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.delete(` http://localhost:6003/api/notes/${noteId}`, { headers });
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      const response = await axios.post(' http://localhost:6003/api/schedules/add', scheduleData, { headers });
