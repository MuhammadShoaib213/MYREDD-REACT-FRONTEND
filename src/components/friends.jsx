...
// Build a full URL for images using the base URL; return as-is if already absolute
...
const FriendsList = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      axios
        .get(`${API_CONFIG.API_URL}/friend/list?userId=${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((response) => {
          if (Array.isArray(response.data)) {
    ...
    <>
      <FriendsContainer>
        {friends.length > 0 ? (
            ...
            <FriendCard key={friend._id}>
              <FriendImageContainer>
                <FriendImage
                  src={
                    friend.profilePicture
                      ? getFullUrl(friend.profilePicture)
                      : "https://via.placeholder.com/200"
                  }
                  alt={friend.firstName}
                />
              ...
              <FriendInfo>
                ...
                <FriendRole>{friend.city}</FriendRole>
                <MoreDetailsButton onClick={() => handleMoreDetails(friend._id)}>
                  More Details
                </MoreDetailsButton>
                <ConnectButton
                  onClick={() =>
          ...
          <NoFriendsMessage>
            Welcome to the Associate Network! You are New Here, Please search your associate if you know his/her ID or send an invitation to join.
          </NoFriendsMessage>
        )}
      </FriendsContainer>
      <ToastContainer position="top-right" autoClose={3000} hideProgressBar />
