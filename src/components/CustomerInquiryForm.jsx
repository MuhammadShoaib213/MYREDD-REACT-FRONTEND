...
const CustomerInquiryForm = () => {
  ...
  //       " http://localhost:6003/api/customers/add",
  //       data,
  //       {
  //         headers: {
  ...
  const handleWhatsAppSubmit = async (e) => {
    ...
    try {
      await axios.post(
        " http://localhost:6003/api/CustomerInvites/create-invite-token",
        { inviteToken },
        {
          headers: {
  ...
  const handleSendOtp = async (emailType) => {
    ...
    try {
      const response = await axios.post(
        " http://localhost:6003/api/auth/send-otp",
        { email }
      );
  ...
  const handleVerifyOtp = async (emailType) => {
    ...
    try {
      const response = await axios.post(
        " http://localhost:6003/api/auth/verify-otp",
        { email, otp }
      );
      if (response.status === 200) {
