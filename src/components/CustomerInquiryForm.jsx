...
import axios from "axios";
import { useNavigate } from "react-router-dom";
// If installed via `npm install jwt-decode`, use default import:
...
const CustomerInquiryForm = () => {
  ...
  // For the rest of your validation errors (non-CNIC)
  const [validationErrors, setValidationErrors] = useState({});
  ...
  // 2) Provide a ToastContainer somewhere in your render tree
  // (One instance per app is enough, but it can be placed here or in App.js)
  // We place it here for convenience
  // You can customize positions, autoClose times, etc.
  ...
  const fetchCities = (countryCode, setCitiesFunc) => {
    fetch(
      `http://api.geonames.org/searchJSON?country=${countryCode}&username=shoaib1&cities=cities1000`
    )
      .then((response) => response.json())
      .then((data) => setCitiesFunc(data.geonames))
      .catch((error) => {
  ...
  const handleChange = (e) => {
    ...
    // If the user is changing the CNIC field:
    if (name === "cnicNumber") {
      ...
      if (!digitsOnly) {
        setCnicError("CNIC is required (16 digits).");
      } else if (digitsOnly.length < 16) {
        setCnicError(
          `Please enter exactly 16 digits. Currently: ${digitsOnly.length}`
        );
      } else {
        setCnicError("");
      }
      ...
      setFormData((prev) => ({
        ...prev,
        cnicNumber: digitsOnly,
      }));
      return; // Exit handleChange because we already handled CNIC
    }
    ...
    // Update the personal and WhatsApp numbers if selection is 'same'
    if (name === "officialMobile") {
      if (mobileSelection.personalMobile === "same") {
        setFormData((prevFormData) => ({
          ...prevFormData,
  ...
  const handleMobileSelectionChange = (e) => {
    ...
    // Auto-update the phone fields based on selection
    setFormData((prevData) => ({
      ...prevData,
      [name]: value === "same" ? prevData.officialMobile : "",
    }));
  };
  ...
  // Validate everything except CNIC (since we do real-time for CNIC)
  const validateInputs = () => {
    const errors = {};
    if (!formData.cityFrom) {
      errors.cityFrom = "City of origin is required";
  ...
  const handleSubmit = async (event) => {
    ...
    // Because we are doing CNIC real-time, if there's any cnicError, block submission
    if (cnicError) {
      toast.error("Please fix CNIC before submitting.");
      // Also store it in validationErrors if you want to show inline
    ...
    const errors = validateInputs();
    if (Object.keys(errors).length > 0) {
      // Show each error as a toast, or just store them inline
      Object.values(errors).forEach((msg) => toast.error(msg));
      setValidationErrors(errors);
      return;
    }

    // Retrieve userId from JWT (if available)
    const token = localStorage.getItem("token");
    let userId = "";
    ...
    // If user selected "same", copy officialMobile to personal/WhatsApp
    if (mobileSelection.personalMobile === "same") {
      formData.personalMobile = formData.officialMobile;
    }
    if (mobileSelection.whatsappMobile === "same") {
    ...
    // Include userId and inviteToken if they exist
    if (userId) {
      data.append("userId", userId);
    }
    if (inviteToken) {
  ...
  const handleWhatsAppSubmit = async (e) => {
    ...
    if (!isValidPhoneNumber(number)) {
      const msg =
        "Please enter a valid WhatsApp number. i.e. +92 3123212456 ";
      setWhatsappNumberError(msg);
      toast.error(msg);
      return;
    }
    setWhatsappNumberError("");
    ...
    if (token) {
      ...
      setServerError("You must be logged in to share the form.");
      toast.error("You must be logged in to share the form.");
      navigate("/login");
      return;
    }

    // Send the invite token to the backend
    try {
      await axios.post(
        "api/CustomerInvites/create-invite-token",
    ...
    const formUrl = `${window.location.origin}/CustomerInquiryForm?inviteToken=${newInviteToken}`;
    const encodedMessage = encodeURIComponent(
      `Hello, I would like to share this form with you: ${formUrl}`
    );
    const whatsappLink = `https://wa.me/${number}?text=${encodedMessage}`;
    ...
    window.open(whatsappLink, "_blank");
    toast.success("WhatsApp link opened in new tab!");
    setWhatsappModalVisible(false);
    setWhatsappNumber("");
  };
  ...
  const handleSendOtp = async (emailType) => {
    const email = formData[emailType];
    if (!email) {
      const errMsg = "Please enter an email address first.";
      setEmailVerificationError((prev) => ({
        ...prev,
        [emailType]: errMsg,
      }));
    ...
    try {
      ...
      if (response.status === 200) {
        const msg = `OTP sent to ${email}. Please check your email.`;
        toast.success(msg);
        setOtpSent((prev) => ({ ...prev, [emailType]: true }));
        setEmailVerificationError((prev) => ({ ...prev, [emailType]: "" }));
      } else {
  ...
  const handleVerifyOtp = async (emailType) => {
    ...
    const otp = emailOtp[emailType];
    if (!otp) {
      const errMsg = "Please enter the OTP sent to your email.";
      setEmailVerificationError((prev) => ({
        ...prev,
        [emailType]: errMsg,
      }));
    ...
    try {
      ...
      if (response.status === 200) {
        const msg = `Email ${email} verified successfully.`;
        toast.success(msg);
        setFormData((prev) => ({
          ...prev,
          [`${emailType}Verified`]: true,
  ...
  return (
    <PageContainer>
      {/* Render the ToastContainer somewhere in your component tree */}
      {toastContainer}
      ...
      <StepContainer>
        <Container style={{ display: "block" }}>
          <h2>Step 1: Basic Details</h2>
          <Form onSubmit={handleSubmit}>
            {/* CNIC Input (16-digit real-time check) */}
            <Input
              type="text"
              name="cnicNumber"
              placeholder="Citizen ID (16 digits)"
              value={formData.cnicNumber}
              onChange={handleChange}
            />
            {/* Real-time error message (if any) */}
        ...
        {currentStep === 2 && (
          <Container style={{ display: "block" }}>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              {/* Official Email Field with Verification */}
              <div>
                <Input
                  type="email"
                  name="officialEmail"
