...
const CheckCustomer = () => {
  ...
  // Handle CNIC changes with a 16-digit check
  const handleCnicChange = (event) => {
    // only keep digits
    const input = event.target.value.replace(/\D/g, '');
    setCnic(input);

    // display error if not exactly 16 digits (and not empty)
    if (input.length !== 16 && input.length > 0) {
      setCnicError('CNIC must be 16 digits.');
    } else {
      setCnicError('');
    }
  };
  ...
  const checkCustomer = async () => {
    // If there's a CNIC error or it's not exactly 16 digits, stop
    if (cnicError || cnic.length !== 16) {
      toast.error('Please enter a valid 16-digit CNIC.');
      return;
    }

    const params = { cnicNumber: cnic };
    if (userId) {
      params.userId = userId;
    }
    ...
    try {
      ...
      if (response.data.exists) {
        ...
        // Display "customer not found" then delay navigate
        toast.error('Customer not found! Navigaing to Add Customer Page...');
        setTimeout(() => {
          navigate('/addCustomer', { state: { cnic } });
        }, 2000); // 2-second delay
      }
  ...
  return (
    <>
      {/* Toastify Container (required once in your app's component tree) */}
      <ToastContainer 
        position="top-right"
        autoClose={5000}
      ...
      <FullScreenContainer>
        ...
        <Container>
          <StyledInput
            ...
            value={cnic}
            onChange={handleCnicChange}
            placeholder="Enter 16-digit CNIC"
          />
          {cnicError && <ErrorMessage>{cnicError}</ErrorMessage>}
