...
//       <MainContent>
//         <DetailImage src={customer.profilePicture ? `${customer.profilePicture}` : 'https://via.placeholder.com/200'} alt={customer.fullName} />
//         <DetailText>
...
function getImageFullUrl(filePath) {
  if (!filePath) return '';
  // If filePath is already absolute, return it
...
const CustomerDetail = () => {
  ...
  useEffect(() => {
    const fetchCustomerDetail = async () => {
      try {
        // Use API_CONFIG.API_URL for API request
        const response = await fetch(`${API_CONFIG.API_URL}/customers/detail/${id}`);
        if (!response.ok) {
          throw new Error('Failed to fetch customer details');
        }
        const data = await response.json();
        ...
        <DetailImage 
          src={
            customer.profilePicture 
              ? getImageFullUrl(customer.profilePicture)
