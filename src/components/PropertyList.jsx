...
// // // New helper functions for image path normalization
...
// //   if (!filePath) return '';
// //   // If already absolute, return it as is.
...
// //                 e.stopPropagation();
// //                 handleWhatsAppShare(
// //                   property.title || property.streetName || 'No Title',
...
//     if (inquiryType.forSale) return { label: 'FOR SALE', color: '#E74C3C' };
//     if (inquiryType.forRent) return { label: 'FOR RENT', color: '#2ECC71' };
//     if (inquiryType.onRent) return { label: 'FOR RENT', color: '#3498DB' };
//     if (inquiryType.forPurchase) return { label: 'FOR PURCHASE', color: '#9B59B6' };
//   } else if (typeof inquiryType === 'string') {
//     const type = inquiryType.toLowerCase();
//     if (type.includes('sale')) return { label: 'FOR SALE', color: '#E74C3C' };
...
// // New helper functions for capturing and uploading card image
...
// // Define ActionButton to be used for sharing actions
// const ActionButton = styled.button`
//   flex: 1;
//   padding: 10px;
//   background-color: #3498db;
...
//           handleWhatsAppShare(property.title || property.streetName || 'No Title', imageUrl);
//         } else if (shareType === 'email') {
//           handleEmailShare(property.title || property.streetName || 'No Title', imageUrl);
//         }
//       } catch (error) {
...
//     return (
//       <PropertyCard ref={cardRef} onClick={() => handleCardClick(property._id)}>
//         <StatusContainer>
//           <StatusIcon className="status-icon" onClick={handleStatusClick}>
//             {STATUS_ICONS[status] || STATUS_ICONS['New']}
//           </StatusIcon>
...
//           <ActionButtonsContainer className="share-buttons">
//             {isSharing && <Spinner />}
//             <ActionButton onClick={(e) => handleShare(e, 'whatsapp')}>
//               WhatsApp
//             </ActionButton>
//             <ActionButton onClick={(e) => handleShare(e, 'email')}>
//               Email
//             </ActionButton>
//           </ActionButtonsContainer>
//         </PropertyInfo>
...
import { Link, useNavigate, useParams } from 'react-router-dom';
// If using jwt_decode package, import as:
...
const ActionButtonsContainer = styled.div.attrs({ className: 'share-buttons' })`
  display: flex;
  gap: 10px;
  margin-top: 15px;
  align-items: center;
...
const PropertyList = () => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        const decoded = jwtDecode(token); // adapt if using 'jwt_decode' default import
        const userId = decoded.userId;
  ...
  const handleSearch = () => {
    ...
    // Filter by inquiryType from URL
    if (inquiryType) {
      filtered = filtered.filter(
        (p) =>
          p.inquiryType &&
          p.inquiryType.toLowerCase() === inquiryType.toLowerCase()
      );
    }

    // Filter by propertyType from URL
    if (propertyType) {
      filtered = filtered.filter(
        (p) =>
          p.propertyType &&
    ...
    // Filter by propertySubType from URL
    if (propertySubType) {
      filtered = filtered.filter((p) => {
        if (typeof p.propertySubType === 'string') {
          return p.propertySubType.toLowerCase() === propertySubType.toLowerCase();
        } else if (typeof p.propertySubType === 'object' && p.propertySubType !== null) {
          return Boolean(p.propertySubType[propertySubType]);
        }
        return false;
      });
    }
    ...
    if (searchTerm.trim() !== '') {
      filtered = filtered.filter((p) =>
        (p.title || p.streetName || '').toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    // Area
    if (selectedArea !== '') {
      filtered = filtered.filter((p) => p.district === selectedArea);
    }
  ...
  const updatePropertyStatus = (propertyId, newStatus) => {
    setFilteredProperties((prev) =>
      prev.map((prop) => (prop._id === propertyId ? { ...prop, status: newStatus } : prop))
    );
    setProperties((prev) =>
      prev.map((prop) => (prop._id === propertyId ? { ...prop, status: newStatus } : prop))
    );
  ...
  const PropertyCardComponent = ({ property, handleCardClick, updatePropertyStatus }) => {
    ...
    // UPDATED handleShare with spinner & fallback
    const handleShare = async (event, shareType) => {
      event.stopPropagation();
      if (!cardRef.current) return;
      ...
      if (shareButtons) {
        shareButtons.style.display = 'none';
        // tiny delay to let DOM update (optional)
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
      ...
      try {
        ...
        console.error('Error sharing property ad image:', err);
      } finally {
