...
// Build a full URL for images using the base URL; if already absolute, return as is.
    ...
    const fetchFriends = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_CONFIG.API_URL}/friend/friends-by-city/${cityName}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (response.data && Array.isArray(response.data)) {
          setFriends(response.data);
        ...
        {friends.map((friend) => (
          <FriendCard key={friend._id}>
            ...
            <FriendInfo>
              ...
              <ConnectButton
                onClick={() =>
                  window.open(`https://wa.me/${friend.whatsappNumber}`, '_blank')
                }
              >
                Connect Now
              </ConnectButton>
